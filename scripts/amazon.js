import { products } from '../data/products.js';
import { cart, addToCart } from '../data/cart.js';
import { formatMoney } from './utils/money.js';

let productHTML = ``;

products.forEach((product) => {
    productHTML  += `
        <div class="product-container">
          <div class="product-image-container">
            <img class="product-image"
              src="${product.image}">
          </div>

          <div class="product-name limit-text-to-2-lines">
            ${product.name}            
          </div>

          <div class="product-rating-container">
            <img class="product-rating-stars"
              src="images/ratings/rating-${product.rating.stars * 10}.png">
            <div class="product-rating-count link-primary">
              ${product.rating.count}
            </div>
          </div>

          <div class="product-price">
            ${formatMoney(product.priceCents)}
          </div>

          <div class="product-quantity-container ">
            <select class="js-product-quantity" data-product-id = "${product.id}">
              <option selected value="1">1</option>
              <option value="2">2</option>
              <option value="3">3</option>
              <option value="4">4</option>
              <option value="5">5</option>
              <option value="6">6</option>
              <option value="7">7</option>
              <option value="8">8</option>
              <option value="9">9</option>
              <option value="10">10</option>
            </select>
          </div>

          <div class="product-spacer"></div>

          <div class="added-to-cart" data-product-id = "${product.id}">
            <img src="images/icons/checkmark.png">
            Added
          </div>

          <button class="add-to-cart-button button-primary js-add-to-cart" data-product-id = "${product.id}">
            Add to Cart
          </button>
        </div> 
    `;
})

document.querySelector('.js-product-grid').innerHTML = productHTML;
document.querySelector('.js-cart-quantity').innerText = cart.length === 0 ? '' : cart.length;

document.querySelectorAll('.js-add-to-cart')
.forEach((button) =>{
  button.addEventListener('click', () =>{
    
    // product name from the button
    const productId = button.dataset.productId;
    
    // get the product quantity from the select element
    const productQuantity = parseInt(document.querySelector(`.js-product-quantity[data-product-id="${productId}"]`).value);

    // show added to cart message
    const addedToCart = document.querySelectorAll('.added-to-cart'); 
    addedToCart.forEach((product) => {
      // console.log(item.dataset.productId);
      if (product.dataset.productId === productId) {
        // console.log('matched');
        product.style.opacity = 1;
        setTimeout(() => {
          product.style.opacity = 0;
        }, 1500);
      }
    });
  
    addToCart(productId, productQuantity);
    
    // update cart quantity in the header
    console.log(cart);
    document.querySelector('.js-cart-quantity').innerText = cart.length; 

    // get product details from productId
    // let productPrice;
    // let productImage;
    // let productName;
    // products.forEach((product) => {
    //   if (product.id === productId) {
    //     productPrice = ((product.priceCents)/100).toFixed(2);
    //     productImage = product.image;
    //     productName = product.name;
    //   }
    // });
  }) 
})
